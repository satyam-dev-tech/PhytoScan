...
aiRouter.post('/agent/investigate', authenticateUser, async (req: Request, res: Response) => {
  try {
    ...
    const result = await runAgentInvestigation(
      ...
      scans as any,
      risks as any,
      userRequest || `Analyze health trajectory and pathogen trends for ${cropWithId.name}`
    );
    ...
    const investigationData = {
      ...
      request: userRequest || `Analyze health trajectory and pathogen trends for ${cropWithId.name}`,
      steps: result.steps,
